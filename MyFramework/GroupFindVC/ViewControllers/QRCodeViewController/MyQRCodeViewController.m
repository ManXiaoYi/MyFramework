...
- (void)viewDidAppear:(BOOL)animated {
    ...
    UIButton *btn1 = [[UIButton alloc]initWithFrame:CGRectMake(50, self.view.frame.size.height - 100, self.view.frame.size.width-100, 40)];
    ...
    //条形码
    self.tView = [[UIView alloc]initWithFrame:CGRectMake( (CGRectGetWidth(self.view.frame)-CGRectGetWidth(self.view.frame)*5/6)/2, 100, CGRectGetWidth(self.view.frame)*5/6, CGRectGetWidth(self.view.frame)*5/6*0.5)];
...
- (void)newCodeChooose {
    ...
    [MYAlertAction showActionSheetWithTitle:@"选择" message:nil chooseBlock:^(NSInteger buttonIdx) {
        if (buttonIdx==0) {
            [weakSelf createQR1];
        }
        else if (buttonIdx == 1) {
